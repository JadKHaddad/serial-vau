            ...
            <v-list class="mb-4">
                <v-list-item v-for="(port, index) in app.managedSerialPorts" :key="port.name">
                    <v-row class="mb-4" align="center">
                        ...
                        <v-icon v-if="port.read_state" :color="port.read_state === ReadState.Read ? 'green' : 'red'"
                            :size="16" class="ml-2">
                    ...
                    <v-chip-group column>
                        <v-chip class="mb-4" v-for="(subscription, subIndex) in port.subscriptions" :key="subIndex"
                            closable v-on:click:close="unsubscribe(port.name, subscription)">
                            {{ subscription }}
                        </v-chip>
                    </v-chip-group>
                    ...
                    <v-list-item-action class="mb-4">
                        ...
                        <v-btn @click="openSerialPort({ name: port.name, initial_read_state: ReadState.Read })"
                            variant="plain">
                            Open
                        </v-btn>
